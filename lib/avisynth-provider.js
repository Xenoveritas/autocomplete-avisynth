...
class Completion {
  constructor(json, type, wiki) {
    if (typeof json !== 'object')
      throw new Error("Bad completion definition " + json);
    this.text = json['text'];
    ...
    if ('signature' in json) {
      if (json['signature'] === '') {
        ...
        // As of now, signatures may also be arrays. These indicate "overloaded"
        // functions. However, for now, simply only use the first one.
        let signature = json['signature'];
        if (typeof signature !== 'string' && typeof signature.forEach === 'function') {
          signature = signature[0];
        }
        // Parse the signature.
        let fields = signature.split(/\s*,\s*/).map((field, index) => {
